...
const state = { query: "", titleQuery: "", group: null, category: null, current: null };
...
function visibleDocuments() {
  ...
  const qt = normalize(state.titleQuery.trim());
  ...
  return documents.filter((doc) => {
    ...
    if (qt && !normalize(doc.title).includes(qt)) return false;
    if (!q) return true;
...
// MEDGUIDE build: specialty menu with submenus
